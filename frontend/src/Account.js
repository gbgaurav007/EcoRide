  ...
  function handleChange(event) {
    ...
    if (!isLogin) {
      ...
      switch (name) {
        ...
        case "contact":
          newErrors.contact =
            validateContact(value) && value.length === 10
              ? ""
      ...
      <div id="account" className="flex flex-col md:flex-row mt-20 md:ml-10">
        <div>
          <h1 className="text-3xl font-bold md:mt-10 mt-10 ml-5 md:ml-12 text-slate-800">
            Share the Ride! <br />
            &emsp;&emsp;&emsp;Share the Journey!
          </h1>
          <img
            src="acc.jpg"
            alt="logo"
            className="w-auto h-50 md:w-auto md:h-80 mx-auto md:ml-12 mt-4 md:mt-8"
          />
        </div>
        <div className="w-3/4 md:w-1/4 md:mt-10 flex flex-col md:ml-20 ml-8 md:py-10">
          ...
          {isLogin ? (
            <p onClick={() => setIsLogin(!isLogin)}>Create an account</p>
          ) : (
            <>
              <div className="flex flex-row relative">
                <input
          ...
          {isLogin ? <p></p> : <button onClick={handleSignup}>Sign Up</button>}
